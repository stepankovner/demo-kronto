...
Яндекс 360 → Домены → krontoai.ru → **Управлять DNS-записями**. Добавьте:
...
| A | `@` | `185.199.110.153` |
| A | `@` | `185.199.111.153` |
| CNAME | `www` | `stepankovner.github.io.` |

⚠️ **MX, TXT (SPF, DKIM) и CNAME `mail` не трогайте** — это записи почты. Если уже есть другие A-записи для `@`, удалите только их.

Проверка (DNS обновляется от 15 минут до нескольких часов): `dig +short krontoai.ru` — должны вернуться четыре IP выше.
